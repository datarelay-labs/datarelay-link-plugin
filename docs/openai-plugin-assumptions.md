...
11. Final MCP submission errors `scan_required`, `annotations_required`, and `justification_required` are separate submission gates. They stay BLOCKED until the owner supplies a tool-scan JSON document. `scan_required` PASS requires `scan_status=success`, a timezone-aware `scanned_at` that is not in the future, the same production MCP URL passed to the checker, and a non-empty `tools` list. An empty `tools` list is FAIL. `annotations_required` and `justification_required` PASS only from that scan when every tool has boolean `readOnlyHint`, `openWorldHint`, and `destructiveHint` and a non-empty justification for each hint. The checker does not contact the production server and does not treat unit tests or source text as a scan.
